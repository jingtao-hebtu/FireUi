...
QString DbData::getDefaultNvr()
{
    QString nvrName = AppConfig::DefaultNvrName;
    QStringList names = DeviceTree::getGroupName();
    if (names.count() > 0) {
        nvrName = names.first();
    }
    return nvrName;
...
int DbData::getIpcIndex(const QString &mediaUrl)
{
    //模糊查找/可能带了分隔符要去掉
    int index = -1;
    QString url = mediaUrl.split("|").first();
    for (int i = 0; i < DbData::IpcInfo_Count; ++i) {
        QString url1 = DbData::IpcInfo_RtspMain.at(i).split("|").first();
        QString url2 = DbData::IpcInfo_RtspSub.at(i).split("|").first();
