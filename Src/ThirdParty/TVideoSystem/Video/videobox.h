...
public:
    ...
    void setWidgets(QWidgetList cur_widgets);

    //设置表格布局
    void setLayout(QGridLayout *grid_layout);

    //设置主菜单子菜单文字标识
    void setMenuFlag(const QString &menu_flag);

    void setActionFlag(const QString &action_flag);

    //设置子菜单可见
    void setVisibles(const QList<bool> &cur_visibles);
