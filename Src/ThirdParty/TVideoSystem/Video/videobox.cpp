...
VideoBox::VideoBox(QObject *parent) : QObject(parent) {
    ...
    types.insert("x",
                 QStringList() << "1_1x1" << "1_4x1" << "1_2x4" << "9_2x4" << "1_3x2" << "1_4x2" << "1_5x2" << "1_6x2"
                               << "1_7x2" << "1_8x2");

    //自定义y布局/主要是一些用户定义的不规则的排列布局/加个y用于区分其他布局/可能有雷同
    types.insert("y", QStringList() << "y_1_2" << "y_1_3" << "y_1_5" << "y_1_8" << "y_1_9" << "y_1_10" << "y_1_12"
                                    << "y_1_16");

    //1_4表示通道1-通道4/前面是通道开始的索引/后面是通道结束的索引
    types.insert("4", QStringList() << "1_4" << "5_8" << "9_12" << "13_16" << "17_20" << "21_24" << "25_28" << "29_32"
                                    << "33_36");
    types.insert("6", QStringList() << "1_6" << "7_12" << "13_18" << "19_24" << "25_30" << "31_36");
    types.insert("8",
                 QStringList() << "1_8" << "9_16" << "17_24" << "25_32" << "33_40" << "41_48" << "49_57" << "57_64");
...
void VideoBox::setLayoutType(const QString &layout_type) {
...
void VideoBox::setWidgets(QWidgetList cur_widgets) {
    this->widgets = cur_widgets;
    this->maxCount = cur_widgets.count();
}

void VideoBox::setLayout(QGridLayout *grid_layout) {
    this->gridLayout = grid_layout;
}

void VideoBox::setMenuFlag(const QString &menu_flag) {
    this->menuFlag = menu_flag;
}

void VideoBox::setActionFlag(const QString &action_flag) {
    this->actionFlag = action_flag;
}

void VideoBox::setVisibles(const QList<bool> &cur_visibles) {
    this->visibles = cur_visibles;
}
...
void VideoBox::change_layout() {
    ...
    QString layout_type = action->property("flag").toString();
    //只有当画面布局类型改变了才需要切换
    if (this->layoutType != layout_type) {
        this->layoutType = layout_type;
        change_layout(type, index);
    }
}
