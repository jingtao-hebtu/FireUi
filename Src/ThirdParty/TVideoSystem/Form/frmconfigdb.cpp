...
{
    {
        ...
        if (dbType == "SQLITE") {
            dbInfo.dbName = DbHelper::getDbDefaultFile("data", AppConfig::LocalDbName);
...
void frmConfigDb::on_btnInit_clicked()
{
    ...
    //数据库脚本名字取数据库名字/要保持一致
    QString sqlName = QString("%1/data/%2.sql").arg(QtHelper::appPath()).arg(AppConfig::LocalDbName);
    QFile file(sqlName);
    ...
    time.start();
    {
        ...
        QString fileName2 = QString("%1/data/error.sql").arg(QtHelper::appPath());
