...
void QtHelperCore::setFormInCenter(QWidget *form, QWidget *centerBaseForm, bool sizeHint) {
    int formWidth = sizeHint ? form->sizeHint().width() : form->width();
    int formHeight = sizeHint ? form->sizeHint().height() : form->height();

    //没有指定参照窗体/表示采用系统桌面屏幕为参照
    QRect screenRect = getScreenRect();
    QRect rect = (centerBaseForm ? centerBaseForm->geometry() : screenRect);

    //计算最终要移动到的位置
    int formX = rect.width() / 2 - formWidth / 2 + rect.x();
    int formY = rect.height() / 2 - formHeight / 2 + rect.y();

    //如果发现窗体移动的位置接近看不见/则移动到屏幕可视范围内
    int x2 = screenRect.width() + screenRect.x();
    if (x2 - formX < 100) {
        formX = x2 - 100;
    }
    int y2 = screenRect.height() + screenRect.y();
    if (y2 - formY < 100) {
        formY = y2 - 100;
    }

    form->move(formX, formY);
}
