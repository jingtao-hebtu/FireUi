...
void AbstractVideoWidget::enterEvent(EnterEvent *) {
...
bool AbstractVideoWidget::eventFilter(QObject *watched, QEvent *event) {
    ...
    if (watched == coverWidget) {
        if (type == QEvent::Paint) {
            ...
            //开启抗锯齿/SmoothPixmapTransform表示图片抗锯齿/开启后会增加CPU占用
...
void AbstractVideoWidget::setWidgetPara(const WidgetPara &widgetPara) {
    ...
    bool needResize = (this->widgetPara.scaleMode != widgetPara.scaleMode ||
                       this->widgetPara.borderWidth != widgetPara.borderWidth ||
                       this->widgetPara.bannerPosition != widgetPara.bannerPosition);
...
void AbstractVideoWidget::hideButton(const QStringList &names) {
    bannerWidget->hideButton(names);
}

void AbstractVideoWidget::hideButton() {
    bannerWidget->hideButton();
}

void AbstractVideoWidget::hideButtonAll() {
    bannerWidget->hideButtonAll();
}
