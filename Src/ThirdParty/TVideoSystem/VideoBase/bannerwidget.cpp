...
int BannerWidget::getLabelWidth() const {
    return label->width();
}
...
void BannerWidget::initButton() {
    ...
    //如果之前存在布局则删除布局/居然只能用delete/而不是deleteLater
    if (this->layout()) {
        delete this->layout();
    }
...
void BannerWidget::hideButton(const QStringList &names) {
            foreach (QPushButton *btn, btns) {
            btn->setVisible(!names.contains(btn->objectName()));
        }
}

void BannerWidget::hideButton() {
    QStringList names = bannerPara.names;
    names.removeOne("banner_btnClose");
    this->hideButton(names);
}

void BannerWidget::hideButtonAll() {
    this->hideButton(bannerPara.names);
}
