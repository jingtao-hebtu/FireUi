...
#include "frmvideowidgetpreview_ui.h"
#include "qthelper.h"
...
void frmVideoWidgetPreview::checkOpen()
{
    //没打开的则弹出个提示
    if (!videoWidget->getIsRunning()) {
        QtHelper::showMessageBoxError(QString("当前地址打不开, 请仔细检查地址是否正常\n%1").arg(url));
        this->close();
    }
}
...
void frmVideoWidgetPreview::open(const QString &url, const QString &flag, int recordTime, const QString &fileName)
{
    ...
    QTimer::singleShot(2000, this, SLOT(checkOpen()));
}
