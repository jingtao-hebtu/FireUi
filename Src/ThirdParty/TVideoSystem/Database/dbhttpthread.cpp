...
QMap<QString, QString> DbHttpThread::getTables1() {
    ...
    tables.insert("t_4_1_mold_progress",
...
QMap<QString, QString> DbHttpThread::getTables2() {
    QMap<QString, QString> tables;
    tables.insert("UserInfo",
                  "UserName,UserPwd,UserType,Permission1,Permission2,Permission3,Permission4,Permission5,Permission6,Permission7,UserMark");
    tables.insert("MaterialInfo",
                  "MaterialID,MaterialNumber,MaterialName,MaterialPara1,MaterialPara2,MaterialPara3,MaterialPara4,MaterialPara5,MaterialPara6,MaterialPara7,MaterialPara8,MaterialPara9,MaterialPara10,MaterialMark");
    tables.insert("MaterialData1", "DataID,MaterialFlag,DataX,DataY");
    tables.insert("MaterialData2", "DataID,MaterialFlag,DataX,DataY");
    tables.insert("PartInfo",
                  "PartID,PartNumber,PartName,MaterialName,PartSize,PartImage,PartImagex,PartModel,PartModelx,PartMark");
    tables.insert("MoldInfo", "MoldID,MoldType,MoldName,MoldState,MoldImage,MoldImagex,MoldModel,MoldModelx,MoldMark");
    tables.insert("QualityInfo",
                  "QualityID,PartName,DefectType,DefectReason,QualityImage,QualityImagex,QualityModel,QualityModelx,QualityMark");
    ...
    tables.insert("ProcedureInfo",
                  "ProcedureID,ProcessPlanName,ProcedureIndex,ProcedureName,ProcedureImage,ProcedureImagex,ProcedureModel,ProcedureModelx,ProcessName,MoldName,ProcedureRequire,ProcedurePara,ProcedureMark");
    return tables;
}
...
void DbHttpThread::finished(QNetworkReply *reply) {
    ...
    if (type == QNetworkAccessManager::GetOperation) {
        ...
        //qDebug() << TIMEMS << tag << rootObj.keys();
        if (tag.isEmpty() || !rootObj.contains("result")) {
            emit error(QString("未知数据( %1 )").arg(QString(buffer)));
            return;
        }
