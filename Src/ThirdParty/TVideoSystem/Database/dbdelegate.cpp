...
QWidget *
DbDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &/*option*/, const QModelIndex &index) const {
...
void
DbDelegate::updateEditorGeometry(QWidget *editor, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    //不执行这个生成的控件在坐标原点
    editor->setGeometry(option.rect);
}

bool DbDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option,
...
void DbDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    //分别处理设置了委托列的
    if (delegateColumn == -255 || index.column() == delegateColumn) {
        if (delegateType == "QCheckBox") {
...
void DbDelegate::drawBg(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
...
void
DbDelegate::drawProgressBar(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    //处理进度条委托
