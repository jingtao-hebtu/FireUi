...
void DbPage::bindData(const QString &sql) {
    ...
    if (labSelectInfo) {
        ...
        labSelectInfo->setText(
                QString("第 %1 页  每页 %2 条  共 %3 页  共 %4 条").arg(pageCurrent).arg(recordsPerpage).arg(
                        pageTotal).arg(recordsTotal));
    }
...
QString DbPage::getPageSql() {
    ...
    if (dbType == DbType_Sqlite || dbType == DbType_MySql) {
        ...
        sql = QString("select a.%1 %2 a join (select %0 %2 %3%4 limit %5,%6) b on a.%0 = b.%0").arg(id).arg(
                selectColumn).arg(sqlFrom).arg(whereSql).arg(sqlOrder).arg(startIndex).arg(recordsPerpage);
        ...
        sql = QString(
                "select b.%0 from (select rowid rn, rownum r %3 where rownum <= %2%4) a, %5 b where a.rn = b.rowid and r > %1%4").arg(
                selectColumn).arg(startIndex).arg(startIndex + recordsPerpage).arg(sqlFrom).arg(sqlOrder).arg(
                tableName);
        ...
        sql = QString("select %1 from (select top %5 %1 from (select top %6 %1 %2 %3%4)%7)%4").arg(selectColumn).arg(
                sqlFrom).arg(whereSql).arg(sqlOrder).arg(count).arg(total).arg(sqlOrder2);
    }
...
void DbPage::setControl(QTableView *tableView,
                        ...
                        QLabel *labRecordsTotal, QLabel *labRecordsPerpage,
                        QLabel *labSelectTime, QLabel *labSelectInfo) {
    setControl(tableView, labPageTotal, labPageCurrent, labRecordsTotal, labRecordsPerpage, labSelectTime,
               labSelectInfo, 0, 0, 0, 0);
}
