...
QString DbHelper::getSelectCountSql(const DbType &dbType, const QString &table,
    ...
    if (dbType == DbType_SqlServer || dbType == DbType_Access) {
        ...
        sql = QString("select %1 from (select %1 from %2 %3%4) where rownum <= %5 order by rownum asc").arg(column).arg(
                table).arg(whereSql).arg(orderSql).arg(count);
    } else {
...
QString DbHelper::getDbDefaultFile(const QString &path, const QString &name) {
    QString dbPath, dbName;
#ifdef Q_OS_WASM
    dbName = "/" + name + ".db";
#else
    dbPath = path.isEmpty() ? "data" : path;
    dbName = name.isEmpty() ? appName() : name;
    dbName = QString("%1/%2/%3.db").arg(appPath()).arg(dbPath).arg(dbName);
#endif
    return dbName;
}
...
QStringList DbHelper::getTables(const QString &dbName, const QString &dbType, QSqlDatabase database) {
    QStringList tables;
    QString flag = dbType.toUpper();
    if (flag == "MYSQL") {
        QString sql = QString("select table_name from information_schema.tables where table_schema = '%1'").arg(dbName);
        ...
        QString sql = QString(
                "select object_name,created from user_objects where object_type = 'TABLE' and created > to_date('%1', 'yyyy-MM-dd')").arg(
                "2022-12-31");
        QSqlQuery query(database);
        if (query.exec(sql)) {
            while (query.next()) {
...
void
DbHelper::getBetweenDate(QString &sql, const QString &dbType, const QString &columnName, const QString &dateTimeStart,
                         const QString &dateTimeEnd, bool varchar) {
    if (varchar) {
