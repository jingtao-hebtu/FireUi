...
enum CompanyType {
    ...
    CompanyType_DaHua = 2,      //浙江大华
    CompanyType_UniView = 3,    //杭州宇视
    CompanyType_HuaWei = 4,     //华为智能
    CompanyType_Tiandy = 5,     //天地伟业

    CompanyType_Normal = 10,    //常规类型
    CompanyType_Other = 11,     //其他类型
    CompanyType_File = 12       //文件类型
};
...
struct UrlPara {
    ...
    //校验时间比如秒数清零
    void checkDataTime() {
        QTime timeStart = dateTimeStart.time();
        QTime timeEnd = dateTimeEnd.time();
        timeStart.setHMS(timeStart.hour(), timeStart.minute(), 0);
        timeEnd.setHMS(timeEnd.hour(), timeEnd.minute(), 0);
        dateTimeStart.setTime(timeStart);
...
public:
    //获取厂家类型名称
    static QStringList getCompany();

    //获取厂家类型编号名称
    static void getCompany(QStringList &listId, QStringList &listName);

    //添加厂家类型到下拉框
    static void loadCompany(QComboBox *cbox, bool file = false);

    //根据厂家类型获取对应的onvif相关字符串
    static void
    getOnvifString(CompanyType company, const QString &ip, bool nvr, int index, QString &ipcType, QString &onvifAddr,
                   QString &profileToken, QString &videoSource);

    //各个厂家的实时及回放视频流字符串(通道和码流默认约定从0开始)
    static QString getRtspUrl(CompanyType company, const QString &userName = QString("admin"),
                              const QString &userPwd = QString("123456"), int channel = 0, int streamType = 0);
