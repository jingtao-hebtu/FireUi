...
//出场入场: fade=in:0:300/fade=out:0:300/第一个数字表示第几帧开始/第二个数字表示持续几帧
//跳帧播放: framestep=5
//同屏显示: tile=3x2:nb_frames=6:padding=7:margin=2
//图片参数: eq=contrast=1.0:brightness=0.8:saturation=1.0:gamma=1.0
//尖锐程度: unsharp=13:13
//浮雕效果: convolution='-2 -1 0 -1 1 1 0 1 2:-2 -1 0 -1 1 1 0 1 2:-2 -1 0 -1 1 1 0 1 2:-2 -1 0 -1 1 1 0 1 2'
...
//当前帧号: drawtext=text='%{n}':fontfile=./wenquanyi.ttf:fontsize=60:fontcolor=#ffffff:x=10:y=10
//二维码  : qrencode=text=www.ffmpeg.org

QString FilterHelper::testFilter = "";
...
QString FilterHelper::getFilter(const OsdInfo &osd, bool noimage) {
    ...
    if (isImage) {
        ...
        //关键字符: localtime表示本地时间 gmtime表示UTC时间  当前帧%{n}
        ...
        if (osd.format == OsdFormat_Date) {
            ...
            text = "%{pts\:hms}";
        } else if (osd.format == OsdFormat_FrameNumber) {
            text = "%{frame_num}" + text;
        }
        ...
        if (bgColor != Qt::transparent) {
            list << QString("box=1:boxcolor=%1@%2:boxborderw=%3").arg(bgColor.name()).arg(bgColor.alphaF()).arg(
                    osd.bgPadding);
        }
